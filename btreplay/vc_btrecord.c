...
/**
 * old_next_io - Retrieve next Q trace from input stream
 * @iip: Per-input file information
 * @spec: IO specifier for trace
 *
 * Returns 0 on end of file, 1 if valid data returned.
 */
static int old_next_io(struct ifile_info *iip, struct io_spec *spec)
{
	ssize_t ret;
	__u32 action;
	__u16 pdu_len;
...
again:
	...
	if (verbose > 1)
		fprintf(stderr, "%2d: %10llu+%10llu (%d) @ %10llx\n",
			...
			(long long unsigned)spec->bytes / 4096LLU,
...
static int next_io(struct ifile_info *iip, struct io_spec *spec)
{
	...
	// 72152174485767 W 12160 pdflush 8 0 302315885 1 1 302239477 1 53 128
	ret = fscanf(iip->ifile, "%lld %c %u %s %u %u %lld %u %u %lld %u %u %u %[a-f0-9 ]",
                        &e.time,
                        &e.dir,
                        &e.pid,
                       	proc,
	...
	spec->time = e.time;
	spec->sector = e.sector;
	spec->bytes = e.size;
	ns = e.size / 4096;
	spec->rw = e.dir == 'R' ? 1 : 0;

	spec->digests = malloc(16 * ns);
	b = buf;
	d = spec->digests;
	for (i = 0; i < ns; i++) {
		for (j = 0; j < 16; j++) {
	...
	if (verbose) {
		fprintf(stderr, "%lld %c %u %s %u %u %lld %u %u %lld %u %u %u -%s-\n",
                        e.time,
                        e.dir,
                        e.pid,
                        proc,
...
//	}
	...
	for (i = 0; i < npkts; i++) {
		if (fwrite(&p[i], sizeof(struct io_pkt), 1, stream->ofp) != 1)
			fatal(stream->file_name, ERR_SYSCALL, "fwrite(pkts) failed\n");

		if (fwrite(p[i].digests, 16 * (p[i].nbytes / 4096), 1, stream->ofp) != 1)
			fatal(stream->file_name, ERR_SYSCALL, "fwrite(pkts) failed\n");
		free(p[i].digests);
		// rkj
	}
	...
	if (verbose) {
		...
		for (i = 0; i < npkts; i++, p++)
			fprintf(stream->vfp, "\t%1d %10llu\t%10llu\n",
				...
				(unsigned long long)p->nbytes / 4096);
	}
}
